...
def fixed_indegree(Ce, Ci, d_ex, d_in, Nexct, Ninhb, W, g):
	...
	delays 	= np.concatenate((d_ex*np.ones(pre_ex.shape[0]), d_in*np.ones(pre_in.shape[0])))
...
def brunel_graph(N, w_ex, g, d_ex, d_in, save_graph=False):
	...
	pre_idx, pos_idx, W, delays = fixed_indegree(Ce, Ci, d_ex, d_in, Nexct, Ninhb, w_ex, g)

	post_list = []
	for i in range(Nexct+Ninhb):
		post_list.append([pos_idx[pre_idx==i],W[pre_idx==i],delays[pre_idx==i]])
