...
def evaluate(post_list):
    #initial conditions
    V = [V_rheo+5.0]*np.ones(N)#np.random.uniform(0.0, V_rheo+1.0, size=N )
...
#-----------------------------------------------------------------------------
#Connecting two neurons: neuron 0 to neuron 1
#-----------------------------------------------------------------------------
N     = 2
I_ext = 300.0
w_ex  = 1000.0
post_list = []
post_list.append([[1], [w_ex], [d_ex]])
post_list.append([])
...
# plt.plot(spk_t[spk_id<=N*0.8],spk_id[spk_id<=N*0.8], '.k', markersize=4.0)
# plt.plot(spk_t[spk_id>N*0.8],spk_id[spk_id>N*0.8], '.r', markersize=4.0)
plt.tight_layout()
plt.show()
